// Wire task creation to the Runner-local executor. Repository paths and secrets
// never cross the Platform boundary.
...
// `ns` scopes the temp path so isolated Runner data roots do not collide.
function setupWorktreeOn(runner: LocalExecutor, ns: string): RepoTaskEnv["setupWorktree"] {
  return async ({ id, mirror, worktree, workBranch, baseBranch, agent }) => {
    ...
    // equivalent hook, so their approval pauses are not visible to Alignyard.
    if ((agent ?? "claude") === "claude") {
      ...
      const hooksTmp = path.join(os.tmpdir(), `alignyard-hooks-${ns}-${id}`, ".claude");

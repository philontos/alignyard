...
- 更新后由 Agent 按新版 Skill Review 和整理现有知识；任何知识语义变化都必须进入 Git diff 和人工 Review。
...
- `framework_version` 表示 Alignyard 管理文件的发布版本；缺失时按 legacy v0 处理，当前最新版为 v1。
...
升级后的 Agent 先阅读新 Skill，再以仓库证据 Review 现有知识。只有证据或用户明确意图支持时才调整正文；涉及产品意图、公共接口、架构边界或兼容性时直接在会话中询问用户。Update Task 只允许修改 `.alignyard/`，运行 `ay validate`、提交并等待人工 Review。

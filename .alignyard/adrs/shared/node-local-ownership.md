...
- Platform 可以保存 branch、commit、session 名、execution 状态与经 `ay sync` 明确上传的工程知识快照；不读取源码树或未提交文件。
...
- 工程知识全文只有在 Agent 或用户显式执行 `ay sync` 后进入 Platform；`.alignyard/` 文档不得包含真实用户、已登记 Repository/Task、设备 token 或部署实例秘密。

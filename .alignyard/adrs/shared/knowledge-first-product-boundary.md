...
- 同一业务概念跨模块、服务或 Repository 时，其权威定义、各边界表示和必须保持的语义不变量属于 `.alignyard/` 的稳定契约；具体转换实现与可执行检查仍属于代码、测试或 Agent Harness。
...
- Alignyard 不接管 Agent Harness 的安装、版本、工具或执行生命周期。Harness 可以引用 `.alignyard/` 文档 ID 来编排和校验，但不得复制项目结论形成第二真源。
...
- 普通 Task 增加通用的跨边界语义对齐：先识别概念与边界，再判断等价、差异或未知；差异进入 Spec 的映射与示例，未知且影响设计时必须向人确认。这一机制不依赖领域专用规则清单。
